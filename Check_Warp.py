# -*- coding: utf-8 -*-
"""Created on Tue Apr 21 11:27:41 2020
@author: Sondre                            """
...
# Folder of folders of ODBs
gofor = Azp                                        
stuff= [f for f in os.listdir(gofor) if not (f.endswith('.bat'))]
#print(stuff)

for g in stuff: #for many folders or one particular in stuff
    #Setting directory
    odb_path =gofor+g+'/'
    ...
    # Hent data
    odb_names = [f for f in os.listdir(odb_path) if ((f.endswith('.odb')) and "1000" not in f)]  # if not f.endswith('.inp')]
    ...
    for u in odb_names:#[0:1]:
        try:
             ...
             for maal in range(0,len(Measurementes)):
                 ...
                 # Sort data
                 PlotData=plottts.sortProfile_points_for_plotting(dotCyl, dotCylm)
                 Inter=[[0,2],[6,8]] # The datacolumns in plotdata used for plotting
                 
                 # Logging deformation behaviour for plotting on each profeil plot
                 ...
                 axs[0, maal].set_xlim([-1500, 200])
                 axs[0, maal].set_xlim([-1300, 000])
                 axs[0, maal].set_ylim([-800, 500])
                 ...
                 # Plotte Legends
                 axs[0, maal].legend(handles=handles, loc='best fit', fontsize=9)
             ...
             np.savez(plot_path+g,
                      ...
                      radz=Radi)
             plt.savefig('D:\\PhD\\Simuleringer\\Azp_plots\\'+g+u[:-4]+'.png')
             plt.close()
        except:
             pass