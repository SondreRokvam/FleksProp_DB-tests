...
for g in gofor[1:2]: #for many folders
    ...
    for i in odb_names:
        ...
        #nodloc=odb.rootAssembly.instances['HYDROWINGTOPLEADING-1-1'].nodeSets
        nodloc=odb.rootAssembly.nodeSets

        Setnames = [nodeset.name for nodeset in nodloc.values() if ('R_' in nodeset.name)]
        #del SJekker
        ...
        for profile in Measurementes:
            ...
            print(Setnames,'\n\n',Measurementes)
            AllNodes = nodloc[Setnames[Measurementes.index(profile)]].nodes
            print('Nods',AllNodes)
            Disp = odb.steps['Step-1'].frames[-1].fieldOutputs['U'].getSubset(region=nodloc[Setnames[Measurementes.index(profile)]]).values

            find_dis_trail_nodes = []
            find_dis_lead_nodes = []

            Acount= 0
            for nod in AllNodes[0]:
                #print nod
